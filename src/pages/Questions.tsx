...
import { Plus, Search, Clock, MessageSquareText, Eye, Loader2, Flame, Hash } from "lucide-react";
...
const Questions = () => {
  ...
  const filtered = questions.filter((q) => {
    ...
    const matchesTag = !filterTag || q.category_tags.includes(filterTag);
                  ...
                  <div className="py-3 px-1 hover:bg-muted/5 transition-colors group">
                    ...
                    {(cats.length > 0 || hashes.length > 0) && (
                      <div className="flex flex-wrap gap-1 mt-1.5">
                        ...
                        {hashes.map((tag) => (
                          <span key={tag} className="px-1.5 py-0.5 rounded text-[9px] font-medium bg-info/10 text-info">
                            {tag}
                          </span>
                        ))}
                      </div>
